 ...
 *
 * script-src is nonce-based. Middleware must put this policy on the *incoming
 * request* as well as the response — Next.js extracts the script nonce from
 * the request CSP header, not from x-nonce alone.
 */
...
function scriptSources(nonce) {
  const sources = ["'self'", `'nonce-${nonce}'`, "'strict-dynamic'"]
  if (process.env.NODE_ENV !== 'production') {
    sources.push("'unsafe-eval'", "'unsafe-inline'") // dev tooling only, never in prod
  }
...
export function buildContentSecurityPolicy(nonce) {
  const directives = [
    ...
    "form-action 'self'",
    scriptSources(nonce),
    "style-src 'self' 'unsafe-inline'", // Tailwind / Radix / recharts inline styles
