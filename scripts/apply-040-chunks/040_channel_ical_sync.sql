-- Apply after 039. Safe to run alone in Supabase SQL Editor.
-- See scripts/apply-038-chunks/README.md for deadlock tips.
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_channel_ical_feeds_export_token ON channel_ical_feeds (export_token);
CREATE INDEX IF NOT EXISTS idx_channel_ical_feeds_hotel ON channel_ical_feeds (hotel_id);
CREATE INDEX IF NOT EXISTS idx_channel_ical_feeds_import_active ON channel_ical_feeds (direction, is_active)
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_reservations_ical_uid_feed
  ON reservations (ical_feed_id, ical_uid)
  WHERE ical_uid IS NOT NULL;

ALTER TABLE channel_ical_feeds ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "owner_manage_channel_ical_feeds" ON channel_ical_feeds;
...
DROP POLICY IF EXISTS "manager_read_channel_ical_feeds" ON channel_ical_feeds;
